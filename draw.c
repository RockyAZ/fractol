...
void	ft_str_window(t_win *win)
{
	...
	mlx_string_put(win->mlx_ptr, win->win_ptr, 120, 20, GREEN, iter =
	ft_itoa(win->thread[0].iter));
	mlx_string_put(win->mlx_ptr, win->win_ptr, 0, 40, BLUE, "THREADS:");
	mlx_string_put(win->mlx_ptr, win->win_ptr, 80, 40, BLUE, thread =
	ft_itoa(THREAD));
	free(iter);
	mlx_string_put(win->mlx_ptr, win->win_ptr, 0, 60, PINK, "ZOOM:");
	mlx_string_put(win->mlx_ptr, win->win_ptr, 50, 60, PINK, iter =
	ft_itoa((int)win->thread[0].zoom));
	free(iter);
	free(thread);
}
